 ...
 *
 * <p>The package name must be provided, along with the application restrictions bundle to set. To
 * clear the application restrictions, an empty bundle should be passed.
 */
public class AppRestrictionsProxyHandler extends Handler {
      ...
      case MSG_CAN_SET_APPLICATION_RESTRICTIONS:
        {
          ...
          responseBundle.putBoolean(
              KEY_CAN_SET_APPLICATION_RESTRICTIONS, isCallerAuthorized(msg.sendingUid));
   ...
   *
   * <p>This permission is persistent until it is later cleared by calling this method with a {@code
   * null} value.
   *
   * <p>The supplied application restriction managing package must be installed when calling this
   * API, otherwise an {@link IllegalArgumentException} will be thrown.
   */
  public static void setApplicationRestrictionsManagingPackage(
    ...
    try {
      ...
      if (packageInfo == null) {
        throw new IllegalArgumentException(
            "Package info could not be retrieved for " + "package " + packageName + ".");
      }
      signatures = packageInfo.signatures;
      if (signatures == null) {
        throw new IllegalArgumentException(
            "Package info did not contain signatures " + "for package " + packageName + ".");
      }
      ...
      throw new IllegalArgumentException(
          "Cannot set "
              + packageName
              + " as application "
              + "restriction managing package as it is not installed.",
          e);
    }
    ...
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putStringSet(APPLICATION_RESTRICTIONS_MANAGING_PACKAGE_SIGNATURES_KEY, signatureSet)
  ...
  private void setApplicationRestrictions(String packageName, Bundle appRestrictions) {
    ...
    devicePolicyManager.setApplicationRestrictions(mAdmin, packageName, appRestrictions);
  }
  ...
  /**
   * Checks that the message sent through the bound service was sent by the same package as declared
   * in {@link #setApplicationRestrictionsManagingPackage(Context, String)}, and that its signature
   * has not changed since it was set.
   *
  ...
  private boolean isCallerAuthorized(int callerUid) {
    ...
    try {
      PackageInfo packageInfo =
          packageManager.getPackageInfo(callingPackageName, PackageManager.GET_SIGNATURES);
      if (packageInfo == null) {
        throw new IllegalArgumentException(
            "Package info could not be retrieved for " + "package " + callingPackageName + ".");
      }
      callingPackageSignatures = packageInfo.signatures;
      if (callingPackageSignatures == null) {
        throw new IllegalArgumentException(
            "Package info did not contain signatures " + "for package " + callingPackageName + ".");
      }
    } catch (NameNotFoundException e) {
