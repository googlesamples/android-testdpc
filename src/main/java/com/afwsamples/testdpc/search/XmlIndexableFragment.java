...
public class XmlIndexableFragment extends BaseIndexableFragment {
  ...
  public XmlIndexableFragment(
      Class<? extends BaseSearchablePolicyPreferenceFragment> fragmentClass, @XmlRes int xmlRes) {
    super(fragmentClass);
    this.xmlRes = xmlRes;
  }
  ...
  public List<PreferenceIndex> index(Context context) {
    ...
    try {
      ...
      while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
        ...
        if (NODE_NAME_PREFERENCE_CATEGORY.equals(nodeName)
            || TextUtils.isEmpty(key)
            || TextUtils.isEmpty(title)) {
          continue;
        }
        PreferenceIndex indexablePreference = new PreferenceIndex(key, title, fragmentName);
